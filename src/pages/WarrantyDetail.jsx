...
const WarrantyDetail = () => {
  ...
  const renderRightColumn = () => {
    ...
    if (viewStep === data.estatus) {
        if (data.estatus === 'creado' || data.estatus === 'activo') return <VendorHandover table="garantias" id={id} onUpdate={fetchDetail} />;
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                            ...
                            {['cambio_fisico', 'reparacion'].includes(data.tipo_resolucion) && (
                                <>
                                    ...
                                    <div><div style={detailLabelStyle}>Entrega a Cliente</div><div style={detailValueStyle}>{resData.persona_entrega || '-'}</div></div>
                                    <div><div style={detailLabelStyle}>Fecha Est. Entrega</div><div style={detailValueStyle}>{formatDate(resData.fecha_entrega)}</div></div>
                                </>
                            )}
                        </div>
                    </div>
